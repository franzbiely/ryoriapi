...
export class DashboardController {
  ...
  async getTransactionByStatusAndMoment(status: string, momentType, branchId: number) {
  ...
  @Get()
  async getDashboardData(@Query('sid') store_Id, @Query('bid') branch_Id): Promise<any> {
    ...
    const totalRevenues = transactions.reduce((prev, cur) => (prev + cur.amount), 0)
    ...
    const transactionsMonthlyServed = await this.getTransactionByStatusAndMoment('served', 'month', branch_Id)
    const transactionsMonthlyAwaitingPayment = await this.getTransactionByStatusAndMoment('awaiting_payment_method', 'month', branch_Id)
    const transactionsMonthlyCancelled = await this.getTransactionByStatusAndMoment('cancelled', 'month', branch_Id)

    const transactionsWeeklyServed = await this.getTransactionByStatusAndMoment('served', 'week', branch_Id)
    const transactionsWeeklyAwaitingPayment = await this.getTransactionByStatusAndMoment('awaiting_payment_method', 'week', branch_Id)
    const transactionsWeeklyCancelled = await this.getTransactionByStatusAndMoment('cancelled', 'week', branch_Id)

    const transactionsTodayServed = await this.getTransactionByStatusAndMoment('served', 'day', branch_Id)
    const transactionsTodayAwaitingPayment = await this.getTransactionByStatusAndMoment('awaiting_payment_method', 'day', branch_Id)
    const transactionsTodayCancelled = await this.getTransactionByStatusAndMoment('cancelled', 'day', branch_Id)
